<div id="dokan-seller-listing-wrap" class="grid-view">
    <div class="seller-listing-content">
        <?php if ($sellers['users']) : ?>
                    ...
                    <li class="dokan-single-seller woocommerce coloum-<?php echo esc_attr($per_row); ?> <?php echo (!$store_banner_id) ? 'no-banner-img' : ''; ?>">
                        <div class="store-wrapper">
                                ...
                                <div class="store-data-container">
                                    <div class="featured-favourite">
                                        <?php if ($is_store_featured) : ?>
                                            <div class="featured-label"><?php esc_html_e('Featured', 'bookory'); ?></div>
                                        <?php endif ?>
                                    ...
                                    <?php if ('on' === $show_store_open_close && 'yes' === $dokan_store_time_enabled) : ?>
                                        <?php if (dokan_is_store_open($seller->ID)) { ?>
                                            <span class="dokan-store-is-open-close-status dokan-store-is-open-status" title="<?php esc_attr_e('Store is Open', 'bookory'); ?>"><?php esc_html_e('Open', 'bookory'); ?></span>
                                        <?php } else { ?>
                                            <span class="dokan-store-is-open-close-status dokan-store-is-closed-status" title="<?php esc_attr_e('Store is Closed', 'bookory'); ?>"><?php esc_html_e('Closed', 'bookory'); ?></span>
                                        <?php } ?>
                                    <?php endif ?>
                                    ...
                                    <div class="store-data <?php echo esc_attr($store_open_is_on); ?>">
                                        ...
                                        <?php if (!empty($store_rating['count'])): ?>
                                            <div class="dokan-seller-rating" title="<?php echo sprintf(esc_attr__('Rated %s out of 5', 'bookory'), esc_attr($store_rating['rating'])) ?>">
                                                ...
                                                <p class="rating">
                                                    <?php echo esc_html(sprintf(__('%s out of 5', 'bookory'), $store_rating['rating'])); ?>
                                                </p>
                                            </div>
                                        <?php endif ?>
                                        ...
                                        <?php if (!dokan_is_vendor_info_hidden('phone') && $store_phone) { ?>
                                            <p class="store-phone">
                                                <i class="bookory-icon-phone-2" aria-hidden="true"></i> <?php echo esc_html($store_phone); ?>
                                            </p>
                                        <?php } ?>
                            ...
                            <div class="store-footer">
                                ...
                                <a href="<?php echo esc_url($store_url); ?>" title="<?php esc_attr_e('Visit Store', 'bookory'); ?>">
            ...
            if ($num_of_pages > 1) {
                ...
                $pagination_args = array(
                    ...
                    'type'      => 'array',
                    'prev_text' => '<i class="bookory-icon-angle-left"></i><span>' . __('Previous', 'bookory') . '</span>',
                    'next_text' => '<span>' . __('Next', 'bookory') . '</span><i class="bookory-icon-angle-right"></i>',
                );
        ...
        <?php else: ?>
            <p class="dokan-error"><?php esc_html_e('No vendor found!', 'bookory'); ?></p>
        <?php endif; ?>
    </div>
</div>
